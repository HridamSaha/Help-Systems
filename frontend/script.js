...
async function startTracking(requestId) {
  async function poll() {
    try {
      const res = await fetch("/api/help/track/" + requestId);
      const data = await res.json();
...
async function loadRequests() {
  try {
    const res = await fetch("/api/help/all");
    const data = await res.json();
...
async function assign(requestId) {
  try {
    await fetch("/api/help/assign/" + requestId, { method: "PUT" });
...
async function resolveReq(requestId) {
  try {
    await fetch("/api/help/resolve/" + requestId, { method: "PUT" });
